...
- 로그인 버튼 하단 문구가 버튼과 겹쳐 보이던 문제 수정
- 로그인 모달 내부를 flex column 구조로 보정
- 로그인 버튼 높이 / 줄바꿈 / 여백 보정
- 로그인 안내 문구와 오류 문구 위치를 static으로 고정
- 작은 화면에서도 로그인창 하단 요소가 겹치지 않도록 보정
- 기존 AI 대전 / 로그인 유지 / 로비 UI 기능 유지


## v6.3.6 - AI 기술 선택 직접 진입 핫픽스

- AI 대전에서 `배틀 시작!` 이후 `턴 처리 중`에 계속 멈추던 문제를 구조적으로 수정
- AI전은 `TEAM_SELECT → TURN_RESOLVE → ACTION_SELECT`가 아니라 `TEAM_SELECT → ACTION_SELECT`로 바로 진입
- `TURN_RESOLVE` 예약 호출 실패 가능성을 제거
- AI전 전용 `forceAIActionSelectNow()` 추가
- AI 기술 선택 단계 진입 시 즉시 타이머와 AI 행동 예약 생성
- 기존 AI 방 정리 / 팀 선택 탈출 / 로그인 유지 기능 유지
...
- P1 팀 선택 완료 + P2 AI 상태를 감지하면 서버가 강제로 AI 팀 준비 확인
- 양쪽 팀 준비가 확인되면 `TURN_RESOLVE`로 즉시 전환 후 배틀 시작
- 팀 선택 직후와 AI 대전 시작 직후에 stuck 탈출 sweep 추가
- 프론트에서 AI전 팀 선택 완료 문구를 `AI가 팀을 준비 중입니다`로 변경
- 기존 AI 방 정리 / UI / 로그인 유지 기능 유지


## v6.3.3 - AI 팀 선택 진행 핫픽스

- AI 대전에서 P1 팀 선택 완료 후 `상대 플레이어 선택 대기` 상태에 멈추던 문제 수정
- AI가 연결만 되고 팀 선택 완료로 인정되지 않는 상태를 보정
- 팀 선택 단계 진입 시 AI 팀 준비 상태 강제 확인
- P1 팀 선택 완료 시 P2가 AI라면 AI 팀 준비를 재확인하고 즉시 배틀 시작 시도
- AI 후보 12마리가 비어 있으면 자동 재생성
- 기존 AI 방 정리 / 로비 UI / 로그인 유지 기능 유지



## v6.3.2 - AI 방 잠김 긴급 핫픽스

- AI 대전 중 P1 유저가 나가면 P2 AI도 즉시 제거
- `P1 빈자리 · P2 AI` 상태를 비정상 잔여 상태로 판단하고 방 자동 초기화
- 로비 상태 생성 시 AI만 남은 방 자동 정리
- AI 대전 시작 전 방 상태를 한 번 더 정리
- 사람이 없는 AI전 방이 잠겨서 재입장/AI 대전이 막히는 문제 수정
- 기존 v6.3 AI 연습전 / 로그인 유지 / 로비 채팅 / 랭킹 기능 유지
